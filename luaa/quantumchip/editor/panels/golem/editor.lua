...
function PANEL:Init( )
	...
	-- self.pTextEntry:SetFocusTopLevel( true )
	...
	if control then
		...
		elseif code == KEY_UP then 
			if self.Caret.x > 1 then
				...
				local length = istable( self.Rows[self.Caret.x] ) and #self.Rows[self.Caret.x][1] or #self.Rows[self.Caret.x]
...
function PANEL:SetArea( selection, text, isundo, isredo, before, after )
	...
	for i = 2, #rows do
		...
		-- table_insert( self.FoldData, start.x + i - 1, false )
...
function PANEL:wordLeft( caret )
	local tFolds = self:ExpandAll( )
	local row = istable( self.Rows[caret.x] ) and self.Rows[caret.x][self.Rows[caret.x].Primary-caret.x] or self.Rows[caret.x] 
	self:FoldAll( tFolds )
	if caret.y == 1 then
		if caret.x == 1 then return caret end
		caret = Vector2( caret.x-1, #self.Rows[caret.x-1] )
		row = istable( self.Rows[caret.x] ) and self.Rows[caret.x][self.Rows[caret.x].Primary-caret.x] or self.Rows[caret.x] 
	end
	local pos = string_match( string_sub( row, 1, caret.y - 1 ), "[^%w]+()[%w ]+[^%w ]*$" )
	caret.y = pos or 1
	return caret
end
...
function PANEL:wordRight( caret )
	...
	if caret.y > #row then
		...
		row = istable( self.Rows[caret.x] ) and self.Rows[caret.x][self.Rows[caret.x].Primary-caret.x] or self.Rows[caret.x] 
		if string_sub( row, 1, 1 ) ~= " " then return caret end
	end
	local pos = string_match( row, "[^%w ]+()[%w ]", caret.y )
...
function PANEL:wordStart( caret )
	local tFolds = self:ExpandAll( )
	local line = istable( self.Rows[caret.x] ) and self.Rows[caret.x][self.Rows[caret.x].Primary-caret.x] or self.Rows[caret.x] 
	self:FoldAll( tFolds )
...
function PANEL:FoldLine( nLine )
	...
	if editor_debug then 
		print( "folding...", FoldLevel )
	end 
	
	for i = nLine + 1, #self.Rows do 
		if self.FoldData[i][1] >= FoldLevel or self.FoldData[nLine][3] then 
			if editor_debug then print( nLine, i, #Data, self.FoldData[i][1], self.FoldData[i][3], "|" .. tostring(self.Rows[i]) ) end 
...
function PANEL:ExpandLine( nLine )
	...
	if editor_debug then print( "unfolding..." ) end 
	
	if Data.Primary == nLine then 
		self.Rows[nLine] = Data[1]
		for i = 2, #Data do 
			if editor_debug then print( nLine, i, nLine+i-1, "|" .. tostring(Data[i]) ) end 
			self.Rows[nLine+i-1] = Data[i]
		end
	else 
		local subfolds = false 
		for i = 1, #Data do 
			if editor_debug then print( Data.Primary, i, Data.Primary+i-1, "|" .. tostring(Data[i]) ) end 
...
function PANEL:Paint( w, h ) 
	local _editor_debug = editor_debug
	editor_debug = false
	...
	surface_SetDrawColor( QC.Editor.CurrentEditorTheme.Background )  
	surface_DrawRect( 0, 0, w, h )
	
	surface_SetDrawColor( QC.Editor.CurrentEditorTheme.Lines )
...
function PANEL:DrawTextUnderlay( w, h )
	...
	if self.Params and not self:HasSelection( ) then 
		surface_SetDrawColor( QC.Editor.CurrentEditorTheme.Selection ) -- TODO: Allow users to configure color and if it is active
		surface_DrawRect( ( self.Params[1].y - self.Scroll.y ) * self.FontWidth + self.BookmarkWidth + self.LineNumberWidth + self.FoldingWidth, ( self.Params[1].x + 1 - self.Scroll.x - self:GetFoldingOffset( self.Params[1].x ) ) * self.FontHeight + 1, self.FontWidth, 1 ) 
		surface_DrawRect( ( self.Params[2].y - self.Scroll.y ) * self.FontWidth + self.BookmarkWidth + self.LineNumberWidth + self.FoldingWidth, ( self.Params[2].x + 1 - self.Scroll.x - self:GetFoldingOffset( self.Params[2].x ) ) * self.FontHeight + 1, self.FontWidth, 1 ) 
	end
end
...
local function MakeFoldButton( self, Row ) 
...
function PANEL:PaintSelection( selection, color, outline )
	...
	color = color or QC.Editor.CurrentEditorTheme.Selection
	...
	for Row = line, endline do 
		...
		local length = (istable(self.Rows[Row]) and #self.Rows[Row][1] or #self.Rows[Row]) - self.Scroll.y + 1
		local LinePos = Row - self.Scroll.x - self:GetFoldingOffset( Row )
...
function PANEL:PaintCursor( Caret ) 
	...
	if self.pTextEntry:HasFocus( ) and self:PositionIsVisible( Caret ) then
		...
		if ( RealTime( ) - self.Blink ) % 0.8 < 0.4 then
			surface_SetDrawColor( QC.Editor.CurrentEditorTheme.Caret )
